...
One of such things is simply reading a file, depending on the language, we can go from being able to read a file in a one-liner, or in very confusing ways, in languages like C.
...
The requests we do to the kernel are called system calls, or syscalls, and they're the way our software can ask for things that it inherently doesn't have access to, such as files.

# The Kernel

The Kernel is essentially the core of our operating system. This is true for both Windows and Unix systems, among others.
**Not all operating systems have a kernel, but those we care about do. There's a good article about Kernel-less operating systems [here](https://dl.acm.org/doi/pdf/10.5555/1082161.1082193) but it's besides the point of this post**
...
Ok, let's start with the actual purpose of this post, which is to read a file in different levels of difficulty. We can start by using a programing language like python, which is pretty high level.
...
**By the way, stdout is seen as a file, so when we print something to the screen, we're basically writing what we want to this "file". stdout, alongside stdin (for inputs), and stderr (for errors) are called streams**

Running the above will yield the following:

![Python 1](/images/python1.png)

So, now I want to show you something pretty cool. Remember when I mentioned that all of these programs would eventually land in the kernel, via syscalls? We can actually see what these syscalls are by using a tool called **strace**.
...
Again, I bet you're getting tired of seeing the same syscalls over and over again! The fun part about C is that you can actually call those functions yourself. Let's do it then.
...
Yikes, ok, so before I even tell you what's going on, note that there are some validations missing, like checking if we were actually able to open the file, by asserting if `fd != -1` and so on...

We're using the `openat()` call now. **AT_FDCWD** tells **openat** to operate under the current working directory. **O_RDONLY** tells **openat** to open the file in readonly mode.
...
Yeah, it's pretty much the same, I think you get the idea now, so let's go deeper to the land of the crazy.

#### Assembly

Allright, this is as far as I can personally go as I still need to gain some wisdom from the gods to go below this level, but let me show you how to read a file the professional way.

I'm using **[nasm](https://nasm.us/)** here, if you're insane you can try other methods. **nasm** is a portable x86 assembler, key word is **portable** :)
...
So, now we know how to read these thing and apply them. Dissecting the snippet above a little more we get:
...
We set the **rsi** register to our stack pointer (the **rsp**), which points to the tippity top of it.
...
**This is why you really can't go faster then assembly :)**
